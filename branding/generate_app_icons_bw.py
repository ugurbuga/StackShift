...
BOARD_SVG_FINAL = "blockwise-board.svg"
BACKGROUND_SVG_FINAL = "blockwise-background.svg"
FEATURE_GRAPHIC_SVG_NAME = "blockwise-feature-graphic-1024x500.svg"
FEATURE_GRAPHIC_PNG_NAME = "blockwise-feature-graphic-1024x500.png"

FULL_SVG_NAME = "blockwise-app-icon.svg"
FOREGROUND_SVG_NAME = "blockwise-adaptive-foreground.svg"
MACOS_FOREGROUND_SVG_NAME = "blockwise-macos-foreground.svg"
...
def rasterize_svg(svg_path: Path, output_png: Path, size: int, height: int | None = None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="blockwise-svg-raster-") as temp_dir:
        temp_path = Path(temp_dir)
        ...
        # 2. Render the square SVG using QuickLook
...
def generate_desktop_assets(full_png: Path, macos_png: Path) -> None:
    ...
    resize_png(full_png, DESKTOP_ICONS / "blockwise.png", 512)
    ...
    with tempfile.TemporaryDirectory(prefix="blockwise-ico-") as ico_temp_dir:
        ico_temp = Path(ico_temp_dir)
        for size in ico_sizes:
            png_path = ico_temp / f"blockwise-{size}.png"
            resize_png(full_png, png_path, size)
            ico_images.append((size, png_path.read_bytes()))

        write_ico(DESKTOP_ICONS / "blockwise.ico", ico_images)

    for name, size in iconset_map.items():
        resize_png(macos_png, iconset_dir / name, size)
    icns_path = DESKTOP_ICONS / "blockwise.icns"
    if icns_path.exists():
        icns_path.unlink()
...
def feature_graphic_svg_content() -> str:
  ...
  <text x="160" y="288" font-family="'NunitoBlack', sans-serif" font-weight="900" font-size="85" fill="url(#textGradient)" text-anchor="middle">BLOCK</text>
  <text x="864" y="288" font-family="'NunitoBlack', sans-serif" font-weight="900" font-size="85" fill="url(#textGradient)" text-anchor="middle">WISE</text>
...
def mosaic_tiles(geometry: dict[str, int]) -> str:
    ...
    for row in range(4):
        for column in range(4):
            x = geometry["logo_x"] + column * (cell_size + gap)
            y = geometry["logo_y"] + row * (cell_size + gap)
            lines.append(empty_slot(x=x, y=y, size=cell_size))
    ...
    for row, cells in enumerate(tile_layout):
        for column, cell in enumerate(cells):
            if cell:
                x = geometry["logo_x"] + column * (cell_size + gap)
                y = geometry["logo_y"] + row * (cell_size + gap)
                lines.append(tile_block(x=x, y=y, color=cell["color"], size=cell_size))

    # T-piece blocks - shifted slightly right (+0.1) and slightly down (+0.1)
    t_blocks = [(1.2, 0.3), (1.2, 1.3), (1.2, 2.3), (2.2, 1.3)]

    # Draw Block Layer
    # Upright piece, aligned with the grid for a clean look
    drag_angle = 0
    off_x = 0
    off_y = 0

    for r, c in t_blocks:
        x = geometry["logo_x"] + c * (cell_size + gap)
        y = geometry["logo_y"] + r * (cell_size + gap)
        lines.append(tile_block(x=x, y=y, color=c_gold, size=cell_size, falling=True, angle=drag_angle, offset_x=off_x, offset_y=off_y))

    # Gold overlay blocks (0.25 opacity) - moved 2 units left (-2 col)
    gold_overlay = [(1, 0), (1, 1), (1, 2), (2, 1)]
    lines.append('  <g opacity="0.25">')
    for r, c in gold_overlay:
        x = geometry["logo_x"] + c * (cell_size + gap)
        y = geometry["logo_y"] + r * (cell_size + gap)
        lines.append(tile_block(x=x, y=y, color=c_gold, size=cell_size))
    lines.append('  </g>')
...
def tile_block(x: int, y: int, color: str, size: int = 150, special: str | None = None, falling: bool = False, angle: float = 0, offset_x: float = 0, offset_y: float | None = None) -> str:
    corner = round(size * 0.21)
    special_art = "" if special is None else special_overlay(x=x, y=y, size=size, kind=special)

    if falling:
        off_y = offset_y if offset_y is not None else size * 0.28
        off_x = offset_x
        cx = x + size / 2
        cy = y + size / 2
        transform = f'transform="translate({cx + off_x}, {cy + off_y}) rotate({angle}) translate({-size/2}, {-size/2})"'
